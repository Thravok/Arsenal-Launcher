...
ThemeManager::ThemeManager()
{
    ...
    // Bundled themes live under :/icons/<id>/ (see *.qrc). Without this path,
    // QIcon::fromTheme falls through inheritance to Multimc PNGs.
    QStringList themePaths = QIcon::themeSearchPaths();
    themePaths.prepend(QStringLiteral(":/icons"));
    themePaths << m_iconThemeFolder.path();
...
void ThemeManager::applyCurrentlySelectedTheme(bool initial)
{
    ...
    if (iconTheme.isEmpty() || m_icons.find(iconTheme) == m_icons.end()) {
        iconTheme = QStringLiteral("lucide");
